...
public class OrderedList_inArraySlots
    ...
    public OrderedList_inArraySlots
        ...
        list_iAS = unordered;
		cost = 0;
        for(int pos=0;pos<list_iAS.size();pos++){
			list_iAS.set(champIndex(list_iAS, pos), 
							        list_iAS.set(pos, 
									list_iAS.get(champIndex(list_iAS, pos))
												 )
									  );
		cost++;
        System.out.println( 
    ...
    private int champIndex( ArrayList<Integer> challengers, int fromStart) {
        	int smallest = Integer.MAX_VALUE;
		int index = fromStart;
		int sindex = -1;
		while(index < challengers.size()){
